...
# --- BULK SUPABASE FETCH WITH MULTITHREADING ---
  ...
  try:
    ...
    if not df_sa.empty:
      ...
      for col in df_sa.columns:
        ...
        elif c_low in ["channel_sku", "item sku code", "sku"]:
          rename_dict[col] = "Channel SKU"
        elif c_low in ["type"]:
          rename_dict[col] = "Type"
        elif c_low in ["brand"]:
          rename_dict[col] = "Brand"
        elif c_low in ["qty", "quantity"]:
          rename_dict[col] = "Qty"
...
# ==================== 📥 3. ADD INVENTORY SHEET ====================
  ...
  with tab1:
    ...
    selected_inward_brand = st.selectbox("🏷️ Select Brand", brand_options)
    scan_qty = st.number_input("Quantity per Scan", min_value=1, value=1, step=1)

    def handle_auto_scan():
      raw_code = st.session_state.get("auto_scanned_code", "").strip()
      if raw_code:
...
# ==================== 📤 4. SALE DATA SHEET ====================
  ...
  s_tab1, s_tab2 = st.tabs(
      ["📸 Auto-Push Scan & Add Sale", "✍️ Manual Single Entry Mode"]
  )

  with s_tab1:
    col_s1, col_s2 = st.columns(2)
    with col_s1:
      scan_sale_type = st.selectbox("Order Type", ["SINGLE", "BUNDLE", "BUNDAL"])
      scan_sale_brand = st.selectbox("Brand Name", ["VIDA LOCA", "YUGNIK"])
    with col_s2:
      scan_sale_qty = st.number_input("Qty Sold", min_value=1, value=1, step=1)
      scan_sale_date = st.date_input("Order Date", date.today())

    def handle_auto_sale_scan():
      raw_code = st.session_state.get("auto_sale_code", "").strip()
      if raw_code:
  ...
  with s_tab2:
    with st.form("single_sale_form", clear_on_submit=True):
      ...
      with col_s1:
        ...
        s_sku = st.text_input("Enter Channel SKU").strip().upper()
      ...
      if submit_sale_single and s_sku != "":
        try:
          ...
          supabase.table("sale_data").insert(sale_payload).execute()
          clear_app_cache()
          st.success("✅ Single Sale Record Inserted Successfully!")
        except Exception as e:
